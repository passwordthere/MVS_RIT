...
def lossandaccuracy(loader, model, factor):
    ...
    with torch.no_grad():
        for i, batchdata in enumerate(loader):
            ...
            ##total loss is the weighted sum of surface loss and dice loss plus the boundary weighted cross entropy loss
...
if __name__ == '__main__':
    ...
    Path2file = 'Semantic_Segmentation_Dataset'
    train = IrisDataset(filepath=Path2file, split='train', transform=transform)
    valid = IrisDataset(filepath=Path2file, split='validation', transform=transform)
    test = IrisDataset(filepath=Path2file, split='test', transform=transform)

    trainloader = DataLoader(train, batch_size=bs, shuffle=True, num_workers=workers)
    validloader = DataLoader(valid, batch_size=bs, shuffle=False, num_workers=workers)
    testloader = DataLoader(test, batch_size=bs, shuffle=False, num_workers=workers)

    #    alpha = 1 - np.arange(1,args.epochs)/args.epoch
    ##The weighing function for the dice loss and surface loss 
    alpha = np.zeros(((epochs)))
    alpha[0:np.min([125, epochs])] = 1 - np.arange(1, np.min([125, epochs]) + 1) / np.min([125, epochs])
    if epochs > 125:
        alpha[125:] = 1
    ious = []
